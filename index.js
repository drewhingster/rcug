...
<body>
    ...
    <script>
        ...
        async function loadAllData() {
            ...
            if (window.cdnLoadErrors && window.cdnLoadErrors.length > 0) {
                const criticalLibs = window.cdnLoadErrors.filter(lib => lib === 'PapaParse');
                if (criticalLibs.length > 0) {
                    document.getElementById('loadingMessage').style.display = 'none';
                    document.getElementById('errorMessage').style.display = 'block';
                    document.getElementById('errorMessage').innerHTML = \`<h3>⚠️ Failed to Load Required Libraries</h3><p>The following critical libraries failed to load: \${criticalLibs.join(', ')}</p><p style="margin-top:15px;">Please check your internet connection and refresh the page.</p><p>If the problem persists, the CDN may be temporarily unavailable.</p>\`;
                    return;
                }
                // Non-critical libraries (jsPDF, html2canvas) - show warning but continue
                console.warn('Some export libraries failed to load:', window.cdnLoadErrors.filter(lib => lib !== 'PapaParse'));
            }
            ...
            try {
                ...
                linkNewMembersData(); // Must be AFTER calculateMemberStats to preserve guest data for new members
        ...
        function processGuests(data) {
            ...
            for (let i = (hdr >= 0 ? hdr : 0) + 1; i < data.length; i++) {
                ...
                // Skip rows that contain "CLUB REGISTER" or other header-like text
                const nameUpper = name.toUpperCase();
                if (nameUpper.includes('CLUB REGISTER')) continue;
                if (nameUpper.includes('QUARTER')) continue;
                if (nameUpper.includes('ATTENDANCE')) continue;
        ...
        function linkNewMembersData() {
            // Link guest attendance data to new members who were inducted mid-year
            // This preserves their pre-induction attendance for accurate tracking
            NEW_MEMBERS_DEC7.forEach(name => {
                const member = members.find(m => m.fullName === name);
                const guest = guests.find(g => g.fullName === name);
                if (member && guest) {
                    // Add guest meetings to their Q1 totals (they attended as guests before induction)
                    ...
                    member.regularMeetings.annual += guest.meetings;
                    member.projects.annual += guest.projects;

                    // Recalculate total meetings for affected periods
                    member.totalMeetings.q1 = member.regularMeetings.q1 + member.committeeMeetings.q1;
                    member.totalMeetings.h1 = member.regularMeetings.h1 + member.committeeMeetings.h1;
                    member.totalMeetings.annual = member.regularMeetings.annual + member.committeeMeetings.annual;
                }
            });
        }
        ...
        function renderMemberCard(m) {
            ...
            if (m.isBoardMember && m.boardMeetings) {
                ...
                const boardPct = btotal > 0 ? Math.round((bp/btotal)*100) : 0;
                boardSection = \`
                    <div class="progress-row">
                        <span class="progress-label">Board Mtgs</span>
                        <div class="progress-bar">
                            <div class="progress-fill board" style="width: \${Math.min(boardPct, 100)}%"></div>
                        </div>
                        <span class="progress-value">\${bp}/\${btotal} (\${boardPct}%)</span>
                    </div>
                \`;
            }
        ...
        function buildAttendanceHistory(member, period) {
            ...
            relevantMeetings.forEach(meeting => {
                const wasAttended = member.attendanceDetails.some(att => {
                    // Must match date
                    if (att.dateKey !== meeting.dateKey) return false;

                    // For committee meetings, must also match the specific committee
                    if (meeting.type === 'Committee Meeting') {
                        return att.type === 'Committee Meeting' &&
                               att.committeeName === meeting.committeeName;
                    }

                    // For other meeting types, just match the type
                    return att.type === meeting.type;
                });
        ...
        function checkPdfLibrary() {
            if (!window.jspdf || !window.jspdf.jsPDF) {
                alert('PDF export is not available. The jsPDF library failed to load. Please check your internet connection and refresh the page.');
                return false;
            }
            return true;
        }
        ...
        async function exportMemberCard(name) {
            ...
            if (m.isBoardMember && m.boardMeetings) {
                ...
                const boardPctPdf = btotal > 0 ? Math.round((bp/btotal)*100) : 0;
                tableData.push(['Board Meetings', bp.toString(), btotal.toString(), \`\${boardPctPdf}%\`]);
            }
