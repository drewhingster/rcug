...
<body>
    ...
    <script>
        ...
        const TOTALS = { q1: { meetings: 6, projects: 5 }, q2: { meetings: 5, projects: 5 }, h1: { meetings: 11, projects: 10 }, elections: { meetings: 3, projects: 0 } };
        ...
        // Committee Assignments for RY 2025-2026
        ...
        function processMembers(data) {
            ...
            for (let i = hdr + 1; i < data.length; i++) {
                ...
                members.push({
                    ...
                    isBoardMember, 
                    isTerminated,
                    meetings: { q1: 0, q2: 0, h1: 0, elections: 0 },
                    projects: { q1: 0, q2: 0, h1: 0, elections: 0 },
                    meetingDetails: { q1: [], q2: [], h1: [], elections: [] },
                    missedMeetings: { q1: [], q2: [], h1: [], elections: [] },
        ...
        function calculateMemberStats() {
            members.forEach(m => {
                ['q1', 'q2', 'h1', 'elections'].forEach(p => {
                    ...
                    m.meetings[p] = att.filter(a => a.type === 'Business').length;
                    m.projects[p] = att.filter(a => a.type === 'Project').length;
                    m.meetingDetails[p] = att.filter(a => a.type === 'Business').map(a => ({ date: a.dateKey, type: a.type }));
                    
                    const allBusinessDates = allAttendance.filter(a => {
                        if (p === 'elections') return a.type === 'Business' && a.quarter === 'Elections Period';
                        if (p === 'h1') return a.type === 'Business' && (a.quarter === 'Q1' || a.quarter === 'Q2');
                        return a.type === 'Business' && a.quarter === p.toUpperCase();
                    }).map(a => a.dateKey).filter((v, i, a) => a.indexOf(v) === i);
                    
                    const attendedDates = new Set(m.meetingDetails[p].map(md => md.date));
                    m.missedMeetings[p] = allBusinessDates.filter(d => !attendedDates.has(d));
                });
            });
        }
        ...
        function renderMemberCard(m) {
            ...
            const yearsInClub = getYearsOfService(m.dateInducted);
            const committees = getCommittees(m.fullName);
            ...
            // Years badge
            const yearsDisplay = yearsInClub !== null && yearsInClub >= 0
                ? \`<span class="badge" style="background:#1abc9c;">\${yearsInClub}yr\${yearsInClub !== 1 ? 's' : ''}</span>\`
                : '';
            
            // Committee tags
            const committeeTags = committees.length > 0 
                ? \`<div style="margin-top:5px;">\${committees.map(c => \`<span style="display:inline-block;background:rgba(243,156,18,0.3);color:#f39c12;padding:2px 6px;border-radius:4px;font-size:0.65rem;margin-right:3px;">\${c}</span>\`).join('')}</div>\`
                : '';
            ...
            return \`
                <div class="member-card \${statusClass}" onclick="showMemberDetails('\${m.fullName.replace(/'/g, "\\\\'")}')">
                    ...
                    <div class="card-header">
                        <div>
                            <div class="member-name">\${m.fullName} \${badges.join('')} \${yearsDisplay}</div>
